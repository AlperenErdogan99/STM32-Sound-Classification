...
#ifndef __AI_MY_MODEL_H__
#define __AI_MY_MODEL_H__
#pragma once

#include "ai_platform.h"
#include "ai_platform_interface.h"

#define AI_MY_MODEL_MODEL_NAME          "my_model"

#define AI_MY_MODEL_ACTIVATIONS_ALIGNMENT   (4)

#define AI_MY_MODEL_IN_NUM       (1)
#define AI_MY_MODEL_IN { \
  AI_BUFFER_OBJ_INIT(AI_BUFFER_FORMAT_FLOAT, 1, 18, 13, 1, NULL), \
}
#define AI_MY_MODEL_IN_SIZE { \
  (1 * 18 * 13), \
}
#define AI_MY_MODEL_IN_1_SIZE  (1 * 18 * 13)
#define AI_MY_MODEL_IN_1_SIZE_BYTES  ((1 * 18 * 13) * 4)





#define AI_MY_MODEL_OUT_NUM      (1)
#define AI_MY_MODEL_OUT { \
  AI_BUFFER_OBJ_INIT(AI_BUFFER_FORMAT_FLOAT, 1, 1, 3, 1, NULL), \
}
#define AI_MY_MODEL_OUT_SIZE { \
  (1 * 1 * 3), \
}
#define AI_MY_MODEL_OUT_1_SIZE  (1 * 1 * 3)
#define AI_MY_MODEL_OUT_1_SIZE_BYTES  ((1 * 1 * 3) * 4)

#define AI_MY_MODEL_N_NODES (6)
 ...
 * @ref ai_my_model_create. Both the activations memory buffer 
 ...
 * using @ref ai_my_model_get_error
 */
AI_API_ENTRY
 ...
 * header generated by the code generation tool. See AI_MY_MODEL_*
 * defines into file @ref my_model.h for all network sizes defines
 *
 * @param network an opaque handle to the network context
 ...
 */
AI_API_ENTRY
ai_i32 ai_my_model_forward(
  ai_handle network, const ai_buffer* input);

AI_API_DECLARE_END

#endif /*__AI_MY_MODEL_H__*/
